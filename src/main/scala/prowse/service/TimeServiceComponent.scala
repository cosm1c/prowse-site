...
trait TimeServiceComponent {
  ...
  trait TimeService {
    ...
    def dateHeader(block: => Result): Result = {
      block.withHeaders(DATE -> timeService.nowAsHttpDate)
    }
  }

}

trait SystemUTCClockTimeServiceComponent extends TimeServiceComponent {

  override val timeService = HardwareClockTimeService

  object HardwareClockTimeService extends TimeService {
